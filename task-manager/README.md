...
### 4. 댓글 & 멘션
태스크 편집 화면 오른쪽 패널에서 댓글을 작성할 수 있습니다.
- `@닉네임`으로 팀원을 멘션하면 자동완성이 뜨고, 대상에게 알림이 전송됩니다.
- 댓글은 태스크 히스토리에도 기록되어 주간보고서에 반영됩니다.
...
### 6. 태스크 아카이브
완료된 태스크를 아카이브하여 보드를 깔끔하게 유지할 수 있습니다.

- 태스크 카드를 오른쪽 끝 **ARCHIVE** 컬럼으로 **드래그 앤 드롭**
- 또는 태스크 편집 모달에서 **Archive** 버튼 클릭
- 아카이브된 태스크는 보드에서 사라지지만 데이터는 유지됨
- **Browse Archived** 버튼으로 아카이브 검색 모달 열기
  - 키워드, 담당자, 기간(1주/1개월/3개월/전체) 필터링
  - 아카이브 해제(Unarchive)하면 다시 보드에 복귀

---

## AI 일감 분해 (Task Generator)

큰 목표나 과업을 입력하면 LLM이 팀원들이 바로 착수할 수 있는 작은 단위의 일감으로 자동 분해합니다.

### 사용 방법
1. 보드 오른쪽 ARCHIVE 컬럼의 **캐릭터를 1초 이내에 3번 클릭**하면 Task Generator 모달이 열립니다.
2. 왼쪽 패널에서 목표/과업을 입력하고 **Generate Tasks** 클릭
3. 오른쪽 패널에 생성된 일감 목록이 체크박스와 함께 표시됨
4. 각 일감의 제목/설명을 **인라인 수정** 가능
5. 원하는 항목만 체크한 뒤 **Create Selected** 클릭 → TODO 컬럼에 일괄 생성

### 재생성 (Refresh)
- 마음에 들지 않는 일감의 체크를 해제한 뒤 **Refresh Unchecked** 클릭
- 체크된 항목은 유지하고, 체크 해제된 항목만 LLM에 "제외할 항목"으로 전달하여 대체 일감을 생성합니다.

### 시스템 프롬프트
- **Agent Instructions (System Prompt)** 토글을 열면 프롬프트를 직접 수정할 수 있습니다.
- 기본값은 JSON 배열 형식으로 5~15개의 일감을 생성하도록 설정되어 있습니다.
- LLM 응답 형식이 맞지 않을 경우 프롬프트를 조정하세요.

### LLM 설정
주간보고서와 동일한 LLM 엔드포인트를 사용합니다. 아래 환경변수가 필요합니다.
...
Markdown 백업 외에 SQLite DB 파일 자체를 다운로드/업로드할 수도 있습니다.

- Switch Workspace 모달 하단의 **Download DB** / **Upload DB** 버튼 사용
- Upload 시 기존 DB는 자동 백업 후 교체됨 (서버 재시작 필요)
- 전체 데이터(모든 워크스페이스, 사용자, 히스토리)가 포함됨

---

## 사용 통계 (Usage Stats)

서버 관리자가 CLI로 사용 현황을 조회할 수 있습니다.
...
출력 항목:
- DB 파일 크기, 총 사용자/워크스페이스/태스크 수
- DAU/WAU (일간/주간 활성 사용자)
- 가입 퍼널 (닉네임 → 워크스페이스 진입 전환율)
- 이탈률 (7일 이상 미접속 비율)
- 참여도 (댓글/알림/보고서/아카이브 건수)
...
- 순수 HTML/CSS/JS 프론트엔드 (빌드 불필요, CDN 불필요, 다크 테마)
- SQLite (설정 없이 바로 사용)
- 선택: FabriX LLM (주간보고서 생성, AI 일감 분해), SMTP (이메일 알림)
